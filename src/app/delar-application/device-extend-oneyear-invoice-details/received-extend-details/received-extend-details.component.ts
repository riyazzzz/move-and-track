...
@Component({
  ...
  templateUrl: "./received-extend-details.component.html",
  styleUrls: ["./received-extend-details.component.scss"],
})
export class ReceivedExtendDetailsComponent implements OnInit {
  @Input() value;
  companyName: any;
  ...
  submitBtn() {
    ...
    const url = serverUrl.web + "/esim/saveEsimExtendOneYearInvoiceDetails";
    this.ajaxService
      ...
      .subscribe((res) => {
        if (
          ...
          res.message == "Extend One Year Invoice Detail Not Saved Successfully"
        ) {
          this.commonService.showConfirm(
            "Extend One Year Invoice Detail Saved Not Successfully"
          );
        } else {
          this.commonService.showConfirm(
