...
export class ReceivedDetailsComponent implements OnInit {
  ...
  submitBtn() {
    ...
    const url = serverUrl.web + "/esim/saveEsimSalesInvoiceDetails";
    this.ajaxService
      .ajaxPostWithString(url, JSON.stringify(data))
      .subscribe((res) => {
        if (
          JSON.parse(res).message == "Sales Invoice Detail Saved Successfully"
        ) {
          this.commonService.showConfirm(
            "Sales Invoice Detail Saved Successfully"
          );
          this.modalController.dismiss({
            data: "Sales Invoice Detail Saved Successfully",
          });
        } else if (
          res.message == "Sales Invoice Detail Not Saved Successfully"
        ) {
          this.commonService.showConfirm(
            "Sales Invoice Detail Not Saved Successfully"
          );
        } else {
          this.commonService.showConfirm(
  ...
  ngOnInit() {
    ...
    const url =
      serverUrl.web + "/esim/getSingleSalesEsimInvoice?headerid=" + this.value;
