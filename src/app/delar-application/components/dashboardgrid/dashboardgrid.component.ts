...
export class DashboardgridComponent implements OnInit, OnChanges {
  ...
  constructor(
    private ajaxService: AjaxService, private loadingCtrl: LoadingController,
  ...
  pendingCADashboard
    ...
    this.columns = [{
      ...
      formatFunction(settings) {
        settings.template = `<span >${settings.value == '' || settings.value == null ? '--' : settings.value}</span >`;
      }
    },
    {
      label: 'ICCID No',
    ...
    let url = serverUrl.web + "/esim/getDealerCAStatusPending?companyid=apm&invoiceno=&serialno=&dealer=" + this.dealer;
